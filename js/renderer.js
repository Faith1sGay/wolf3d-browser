...
Wolf.Renderer = (function () {
  var slices = []
  var useBackgroundImage = Wolf.ISWEBKIT
  var texturePath = 'art/walls-shaded/' + Wolf.TEXTURERESOLUTION + '/'
  var spritePath = 'art/sprites/' + Wolf.TEXTURERESOLUTION + '/'
  var sprites = []
  var maxDistZ = 64 * 0x10000
  var hasInit = false
  visibleSprites = []

  var TILESHIFT = Wolf.TILESHIFT
  var TILEGLOBAL = Wolf.TILEGLOBAL
  var TRACE_HIT_VERT = Wolf.TRACE_HIT_VERT
  var TRACE_HIT_DOOR = Wolf.TRACE_HIT_DOOR
  var WALL_TILE = Wolf.WALL_TILE
  var DOOR_TILE = Wolf.DOOR_TILE
  var TEX_PLATE = Wolf.TEX_PLATE
  var TILE2POS = Wolf.TILE2POS
  var POS2TILE = Wolf.POS2TILE
  var VIEW_DIST = Wolf.VIEW_DIST
  var SLICE_WIDTH = Wolf.SLICE_WIDTH
  var WALL_TEXTURE_WIDTH = Wolf.WALL_TEXTURE_WIDTH
  var FINE2RAD = Wolf.FINE2RAD
  var XRES = Wolf.XRES
  var YRES = Wolf.YRES
  var MINDIST = Wolf.MINDIST
  var cos = Math.cos
  var sin = Math.sin
  var tan = Math.tan
  ...
  function processTrace (viewport, tracePoint) {
    ...
    var dx = viewport.x - tracePoint.x
    var dy = viewport.y - tracePoint.y
    var dist = Math.sqrt(dx * dx + dy * dy)
    var frac
    var h; var w; var offset
  ...
  function updateSlice (n, textureSrc, proc) {
    var slice = slices[n]
    var image = slice.texture
    var sliceStyle = slice.style
    var imgStyle = image.style
    var top = (Wolf.YRES - proc.h) / 2
    var left = -(proc.offset) >> 0
    var height = proc.h
    var z = (maxDistZ - proc.dist) >> 0
    var itop
  ...
  function drawWall (n, viewport, tracePoint, level) {
    ...
    var y = tracePoint.tileY
    var vx = POS2TILE(viewport.x)
    var vy = POS2TILE(viewport.y)
    var tileMap = level.tileMap
    var proc = processTrace(viewport, tracePoint)
    var texture = proc.vert ? level.wallTexX[x][y] : level.wallTexY[x][y]
    var textureSrc
