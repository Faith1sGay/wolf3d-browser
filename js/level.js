...
Wolf.Level = (function () {
  ...
  function readPlaneData (file, offset, length, rle) {
    ...
    var carmackData = Wolf.File.readBytes(file, length - 2)
    var expandedData = carmackExpand(carmackData, expandedLength)
    ...
    for (y = 0; y < 64; ++y) {
      for (x = 0; x < 64; ++x) {
        ...
        switch (tile) {
          ...
          case 254:
          case 255:
            if (skill < Wolf.gd_hard) { break }
            tile -= 18
          case 234:
          case 235:
          case 236:
          case 237:
            if (skill < Wolf.gd_medium) { break }
            tile -= 18
          case 216:
          case 217:
          case 218:
  ...
  function checkLine (x1, y1, x2, y2, level) {
    var xt1; var yt1; var xt2; var yt2 /* tile positions */
    var x; var y /* current point in !tiles! */
    var xdist; var ydist
    var xstep; var ystep /* Step value for each whole xy */

    var deltafrac /* current point in !1/256 of tile! */

    var frac /* Fractional xy stepper */

    var partial /* how much to move in our direction to border */
    var intercept /* Temp for door code */

    var FRACBITS = 8 /* Number of bits of fraction */
