...
Wolf.Game = (function () {
  ...
  var rendering = false
        ...
        var paused = false
        var intermissionAnim = 0
        var currentGame = null
        var levelMusic
        var processAI = true
        var keyInputActive = false
        var preloadTextures = {}
        var preloadSprites = {}
  ...
  function startGameCycle (game) {
    var deathTics = 0
            var deathTicsMax = ticsPerSecond * 2
  ...
  function calcTics () {
    var now = (new Date()).getTime()
            var delta = (now - lastTimeCount) / 1000
            var tics = Math.floor(ticsPerSecond * delta)
  ...
  function updateHUD (game, tics) {
    var player = game.player
            var frame = player.weapon * 4 + player.weaponFrame
  ...
  function updateFPS () {
    var now = (new Date()).getTime()
            var dt = (now - lastFPSTime) / 1000
            var frames = frameNum - lastFrame
  ...
  function preloadLevelAssets (level, callback) {
    var files = []
            var tx; var ty; var texture; var x; var y; var f; var i; var numFiles
            var texturePath = 'art/walls-shaded/' + Wolf.TEXTURERESOLUTION + '/';
    var spritePath = 'art/sprites/' + Wolf.TEXTURERESOLUTION + '/'
  ...
  function startIntermission (game, delay) {
    var episode = Wolf.Episodes[game.episodeNum]
            var parTime = episode.levels[game.levelNum].partime * 60
            var bonus = 0
            var parBonusAmount = 500
            var ratioBonusAmount = 10000
            var levelState = game.level.state
            var killRatio = levelState.totalMonsters ? ((levelState.killedMonsters / levelState.totalMonsters * 100) >> 0) : 0
            var secretRatio = levelState.totalSecrets ? ((levelState.foundSecrets / levelState.totalSecrets * 100) >> 0) : 0
            var treasureRatio = levelState.totalTreasure ? ((levelState.foundTreasure / levelState.totalTreasure * 100) >> 0) : 0
            var time = levelState.elapsedTime + ((new Date()).getTime() - levelState.startTime)
            var totalTime; var i  
            var avgKill = 0; var avgSecret = 0; var avgTreasure = 0
    ...
    function anim () {
      var now = (new Date()).getTime()
                var bjFrame = Math.floor(now / 500) % 2
  ...
  function enterFullscreen () {
    var ratio = window.innerWidth / 640
            var sliceZoom = Math.floor(Wolf.SLICE_WIDTH * ratio)
            var zoom = sliceZoom / Wolf.SLICE_WIDTH
            var transform = 'scale(' + zoom + ')'
